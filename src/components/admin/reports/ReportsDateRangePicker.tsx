...
export default function ReportsDateRangePicker({ open, applied, onClose, onApply }: {
    ...
    const appliedDraft = () => applied.key === "custom" && !applied.allTime
        ? { start: applied.start, end: applied.end, allTime: false }
        : { start: today, end: today, allTime: applied.key === "custom" && applied.allTime };
    const [draft, setDraft] = useState(appliedDraft);
    const { start, end, allTime } = draft;
    const dialogRef = useRef<HTMLDivElement>(null);
    useEffect(() => {
        if (open) setDraft(appliedDraft());
        // Applied URL state deliberately wins over an in-progress draft after Back/Forward.
    ...
    if (!open) return null;
    const error = allTime ? null : !start || !end ? "กรุณาเลือกวันเริ่มต้นและวันสิ้นสุด"
    ...
    const chooseMonth = (offset: number) => {
        ...
        setDraft({ start: `${month}-01`, end: offset === 0 ? today : lastDay(month), allTime: false });
    };
    return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/45 p-0 sm:items-center sm:p-4" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
        <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="reports-range-title" tabIndex={-1} className="w-full max-w-md rounded-t-2xl bg-[var(--surface)] p-5 shadow-2xl sm:rounded-2xl">
            <h2 id="reports-range-title" className="text-lg font-bold text-[var(--text-primary)]">กำหนดช่วงเวลารายงาน</h2>
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <label className="text-sm font-semibold text-[var(--text-secondary)]">ช่วงเริ่มต้น<input type="date" value={start} max={end || today} onChange={(e) => setDraft({ start: e.target.value, end, allTime: false })} aria-describedby="reports-range-error" className="mt-2 min-h-11 w-full rounded-xl border border-black/15 bg-[var(--background)] px-3 dark:border-white/15"/></label>
                <label className="text-sm font-semibold text-[var(--text-secondary)]">ช่วงสิ้นสุด<input type="date" value={end} max={today} onChange={(e) => setDraft({ start, end: e.target.value, allTime: false })} aria-describedby="reports-range-error" className="mt-2 min-h-11 w-full rounded-xl border border-black/15 bg-[var(--background)] px-3 dark:border-white/15"/></label>
            </div>
            ...
            <div className="mt-3 grid grid-cols-3 gap-2" aria-label="ทางลัดช่วงเวลา">
                ...
                <button type="button" aria-pressed={allTime} onClick={() => setDraft({ start: today, end: today, allTime: true })} className="min-h-11 rounded-xl border border-black/10 px-1 text-sm font-semibold dark:border-white/15">ตั้งแต่เริ่มใช้</button>
            </div>
            <div className="mt-6 flex justify-end gap-2"><button type="button" onClick={onClose} className="min-h-11 rounded-xl px-4 font-semibold">ยกเลิก</button><button type="button" disabled={error !== null} onClick={() => onApply(allTime ? { key: "custom", start: null, end: null, allTime: true } : { key: "custom", start, end, allTime: false })} className="min-h-11 rounded-xl bg-[var(--accent)] px-4 font-bold text-white disabled:cursor-not-allowed disabled:opacity-45">ดูรายงาน</button></div>
        </div>
    </div>;
}
