        ...
        {loading ? <LoadingSkeleton/> : error ? <Card><div className="flex items-start gap-3"><AlertTriangle className="shrink-0 text-red-600 dark:text-red-300"/><div><h2 className="font-bold text-[var(--text-primary)]">เปิดรายงานไม่ได้</h2><p className="mt-1 text-sm text-[var(--text-muted)]">{error}</p><button type="button" onClick={retry} className="mt-4 min-h-10 rounded-xl bg-[var(--accent)] px-4 text-sm font-bold text-white">ลองใหม่</button></div></div></Card> : data ? <>
            ...
            {empty ? <Card><div className="flex items-start gap-3"><ReceiptText className="shrink-0 text-[var(--accent)]"/><div><h2 className="font-bold text-[var(--text-primary)]">ยังไม่มียอดขายที่ชำระแล้วในช่วงนี้</h2><p className="mt-1 text-sm text-[var(--text-muted)]">ลองเลือกช่วงเวลาอื่น หรือตรวจสอบสาขาที่กำลังดู</p></div></div></Card> : <>
                ...
                <section className="min-w-0 space-y-3" aria-labelledby="daily-sales-heading">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><SectionHeading id="daily-sales-heading" title="ยอดขายตามวัน" description="ดูยอดเงินในแต่ละวันผ่านปฏิทินหรือกราฟแนวโน้ม"/><div className="inline-flex self-start rounded-xl bg-black/5 p-1 dark:bg-white/10" role="group" aria-label="เลือกรูปแบบยอดขายตามวัน"><button type="button" aria-pressed={salesView === "calendar"} onClick={() => setSalesView("calendar")} className={`min-h-9 rounded-lg px-4 text-sm font-bold ${salesView === "calendar" ? "bg-[var(--surface)] text-[var(--accent)] shadow-sm" : "text-[var(--text-muted)]"}`}>ปฏิทิน</button><button type="button" aria-pressed={salesView === "graph"} onClick={() => setSalesView("graph")} className={`min-h-9 rounded-lg px-4 text-sm font-bold ${salesView === "graph" ? "bg-[var(--surface)] text-[var(--accent)] shadow-sm" : "text-[var(--text-muted)]"}`}>กราฟ</button></div></div>
