...
import { resolveBranchContext, resolveShopContext } from "@/lib/contextPolicy.mjs";
...
export async function GET(request: Request) {
    ...
    const { data: memberships } = await admin.from("shop_members").select("shop_id,role,created_at").eq("user_id", auth.user.id).order("created_at");
    ...
    const { data: branches } = await admin.from("branch").select("id,is_primary,created_at").eq("shop_id", shopId).order("created_at");
...
async function clearAndRedirect(request: Request, href: string, userId: string, shopId: string | null, branchId: string | null) {
    await getSupabaseAdmin().from("profiles").update({ current_shop_id: shopId, current_branch_id: branchId }).eq("id", userId);
